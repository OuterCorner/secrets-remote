...
exports.requestSecret = async function (chatServiceAddr, staticKeyPair, pushService, devices, query, callbacks) {
    ...
    const { pushId, notifiedDevices } = await notifyDevices(pushService, devices, {
        peer_id: myId,
        channel: "chatws",
        type: "secret",
    ...
    const peerHello = await peerMessenger.onceMessageReceived({type: "hello", role: "request"})
    ...
    // say hello back
    cc.sendMessage({channel: "setup", type: "hello", role: "response", messageId: peerHello.messageId })
...
async function notifyDevices(pushService, devices, payload) {
