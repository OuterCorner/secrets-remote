...
const { mergeTypedArrays, DeferredPromise } = require('./util');
...
class NoiseSession extends EventEmitter {
	...
	set state(newState) {
		...
		this.emit(NoiseSessionEvents.STATE_CHANGED, this._state)
	}
	...
	_receivedPayload(payload) {
		if (this.state == NoiseSessionState.HANDSHAKING) {
			...
			this.emit(NoiseSessionEvents.MESSAGE_RECEIVED, message)
		}
	}
