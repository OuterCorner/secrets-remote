...
exports.pairDevice = async function (serverAddr, staticKeyPair, callback) {
    ...
    const pskB64 = base64js.fromByteArray(randomBytes)

    // setup noise session
    const noiseLib = await getNoiseLib()
    let noiseSession = new NoiseSession(noise, "NoisePSK_XX_25519_ChaChaPoly_SHA256", noise.constants.NOISE_ROLE_RESPONDER, function(handshake) {
		handshake.Initialize(null, staticKeyPair.priv, null, psk)
    });

    noiseSession.on(Constants.NoiseSessionEvents.STATE_CHANGED, function() {
		console.debug("Session state: " + session.state)
	})

	noiseSession.on(Constants.NoiseSessionEvents.MESSAGE_RECEIVED, function(m) {
		console.log("Message received: " + JSON.stringify(m))
		session.sendMessage({"type": "appContextResponse", "role": "response", "messageId": m.messageId, "contexts": []})
	})
    noiseSession.start()
    ...
    callback({
        peerId: myId,
        secret: pskB64,
        url: `secrets://rsr/pair?requestor-id=${encodeURIComponent(myId)}&pairing-secret=${encodeURIComponent(pskB64)}&channel=chatws`
    })
