...
class PeerMessenger extends EventEmitter {
    ...
    handleOutgoingNoiseData(chunk) {
        ...
        this.chatClient.request({ type: "directMessage", peerId: peerId, message: { channel: "noise", payload: b64Message }})
            .then(result => {
    ...
    sendKeepAlive() {
        ...
        if (peerId) {
            this.chatClient
                .request({ type: "directMessage", peerId: peerId, message: { channel: "keepalive" }})
                .then( (response) =>  {
